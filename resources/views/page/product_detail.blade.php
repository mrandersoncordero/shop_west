      ...
      <article class="container_image--product">
        ...
        <section class="content_detail--product">
              ...
              <div>
                @php
                  ...
                  if ($product->weight == 20) {
                      $paleta = "90 sacos, $product->weight kg c/u.";
                  } elseif ($product->weight == 14) {
                      $paleta = "132 sacos, $product->weight kg c/u.";
                  } elseif ($product->weight > 9 && $product->weight <= 11) {
                      $paleta = "150 sacos, $product->weight kg c/u.";
                  }

                  if ($product->subcategory->id == 7 && $product->id == 22) {
                      $paleta = '48 bultos';
                  }
                @endphp
                <p><span>Tipo de venta: </span> {{ "paleta de $paleta" }}</p>
              </div>
            </div>
            ...
            @auth
              <form action="{{ route('cart.add', $product->id) }}" method="POST" class="container_add--truck">
                ...
                <button type="submit" class="margin-top-content">
                  <div><img src="{{ asset('icons/camion_compra.png') }}" alt="">
          ...
          <div style="display: flex; align-items: center; gap: 4px;">
            <button type="button" class="btn" style="background-color: var(--blue); color: #fff;"
            ...
            @if ($product->palette_color != null || $product->palette_color != '')
              <button type="button" class="btn" style="background-color: var(--blue); color: #fff;"
                data-bs-toggle="modal" data-bs-target="#ModalPaleta">
                Paleta de colores
              </button>
        ...
        <section style="padding: 8px;">
          ...
          @auth
            ...
            {{-- formulario --}}
            <form action="{{ route('products.rate', ['productId' => $product->id]) }}" method="POST"
              style="margin-top: 8px;">
              @csrf
              <div class="form-floating">
                <select name="rating" class="form-select form-select-lg mb-3" id="floatingSelect"
              ...
              <div class="form-floating mb-3">
                <textarea class="form-control" name="comment" placeholder="Escribe tu comentario aquí" id="floatingTextarea"></textarea>
                <label for="floatingTextarea">Comentario</label>
              </div>

              <button type="submit" class="btn btn-primary mb-3">Calificar</button>
            </form>
          @endauth
