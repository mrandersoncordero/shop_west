<nav class="navbar">
  <div class="top-navbar">
    <div>
      <span><a href="mailto:ventasnit@pegoccidente.com" target="_blank"><i
            class="fa-solid fa-envelope"></i>ventasnit@pegoccidente.com</a></span>

      <a style="padding-right: 12px;" href="https://api.whatsapp.com/message/FKHYQ5DCEGCQM1" target="_blank"> <i
          class="fa-brands fa-whatsapp"></i> Trujillo +58 416-5790964</a>

      <span><a href="mailto:ventasnib@pegoccidente.com" target="_blank"><i
            class="fa-solid fa-envelope"></i>ventasnib@pegoccidente.com</a></span>

      <a href="https://wa.me/+584145769904" target="_blank"> <i class="fa-brands fa-whatsapp"></i>Barquisimeto +58
        414-5769904</a>
    </div>
    <div>
      <a href="https://www.facebook.com/ProdOccidente" target="_blank" aria-label="Facebook Productos Occidente"><i
          class="fa-brands fa-facebook"></i></a>
      <a href="https://www.instagram.com/prodoccidente?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw=="
        target="_blank" aria-label="Instagram Productos Occidente"><i class="fa-brands fa-instagram"></i></a>
    </div>
  </div>
  <div class="navbar-content">
    <a href="{{ route('index') }}" aria-label="Inicio"><img src="{{ asset('images/logo.png') }}" alt="Logo"
        class="nav-img"></a>
    <ul class="ul">
      <li><a href="{{ route('about') }}" class="underline">Nosotros</a></li>
      <li><a href="{{ route('news.index') }}" class="underline">Noticias</a></li>
        ...
        <div class="submenu">
          @foreach ($categories as $category)
            <div class="submenu-categorias">
              <a href="{{ route('products_by_category', $category) }}" class="title-category">Linea de
                {{ $category->name }} <i class="fa-solid fa-angles-right"></i></a>
              <div class="submenu-subcategoria">
                @foreach ($category->subcategories as $subcategory)
                  <div>
                    <header><a href="{{ route('products_by_subcategory', $subcategory) }}"
                        class="title-subcategory">{{ $subcategory->name }} <i class="fa-solid fa-angle-right"></i></a>
                    </header>
                    <ul>
                      @foreach ($subcategory->products as $product)
                        <li><a href="{{ route('product_detail', $product) }}"
                            style="text-transform: uppercase"><ion-icon name="radio-button-on-outline"
                              class="icon-diana"></ion-icon>{{ $product->name }}</a></li>
                      @endforeach
                    </ul>
                  </div>
                @endforeach
        ...
        @foreach ($categories as $category)
          <li class="mobile-menu" id="submenu-down">
            ...
            <ul class="mobile-submenu">
              @foreach ($category->subcategories as $subcategory)
                <li><a href="{{ route('products_by_subcategory', $subcategory) }}"
                    class="underline">{{ $subcategory->name }}</a></li>
              @endforeach
            </ul>
          </li>
        @endforeach
